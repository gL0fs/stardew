...
bool GameTimeUI::initUI() {
    ...
    m_dateLabel->setAnchorPoint(Vec2(0, 1));  // 设置为左上角对齐
    m_dateLabel->setPosition(Vec2(30, -10));   // 相对于背景图片的位置
    ...
    m_timeLabel->setAnchorPoint(Vec2(0, 1));  // 设置为左上角对齐
    m_timeLabel->setPosition(Vec2(30, -35));   // 相对于背景图片的位置
    ...
    m_progressBar->setPosition(Vec2(30, -60)); // 相对于背景图片的位置
...
void GameTimeUI::timeupdate(float dt) {
    // 计算每秒应该增加的游戏分钟数
    // 3分钟 = 180秒真实时间对应24小时 = 1440分钟游戏时间
    // 所以每秒需要增加的游戏分钟数是：1440 / 180 = 8分钟
